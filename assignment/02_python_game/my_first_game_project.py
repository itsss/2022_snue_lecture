...
background_img = pg.?????.????("background.png")
background_img = pg.?????????.?????(background_img, (???, ???))

# ===========================================================================
# TODO 7. character에 캐릭터 이미지를 넣고, 가로(100) X 세로(100) 으로 바꾸어 주세요.
character = pg.?????.????("character.png")
...
while running:
    ...
    for event in pg.?????.???():
        ...
        elif event.type == pg.???????: # 키가 눌렸을 때: KEYDOWN
    ...
    screen.????(background_img, background_img.get_rect())
    screen.????(character, character_pos)
